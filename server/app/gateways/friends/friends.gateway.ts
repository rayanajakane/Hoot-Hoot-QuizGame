...
export class FriendsGateway {
    ...
    handleRequestSent(@ConnectedSocket() socket: Socket, @MessageBody() data: { from: string; to: string }) {
        this.server.emit(FriendsEvents.RequestSent, data);
    }

    @SubscribeMessage(FriendsEvents.RequestAccepted)
    handleRequestAccepted(@ConnectedSocket() socket: Socket, @MessageBody() data: { user: string; friend: string }) {
        this.server.emit(FriendsEvents.RequestAccepted, data);
    }

    @SubscribeMessage(FriendsEvents.RequestRejected)
    handleRequestRejected(@ConnectedSocket() socket: Socket, @MessageBody() data: { user: string; friend: string }) {
        this.server.emit(FriendsEvents.RequestRejected, data);
    }

    @SubscribeMessage(FriendsEvents.RequestCanceled)
    handleRequestCanceled(@ConnectedSocket() socket: Socket, @MessageBody() data: { user: string; friend: string }) {
        this.server.emit(FriendsEvents.RequestCanceled, data);
    }

    @SubscribeMessage(FriendsEvents.FriendRemoved)
    handleFriendRemoved(@ConnectedSocket() socket: Socket, @MessageBody() data: { user: string; friend: string }) {
